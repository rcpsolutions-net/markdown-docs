...
graph TD
    subgraph "Indexing Phase"
        A[Unstructured Data<br>(e.g., 'The cat sat on the mat')] --> B(Embedding Model);
        B --> C{Vector<br>[0.1, 0.9, 0.4, ...]};
        C --> D[(Vector Database)];
    end

    subgraph "Query Phase"
        E[Query<br>(e.g., 'Where did the feline sit?')] --> F(Embedding Model);
        F --> G{Query Vector<br>[0.2, 0.8, 0.5, ...]};
